...
constexpr char escape_marker = special_chars[2];
...
std::optional<std::tuple<std::string_view, size_t>> attempt_unescape(std::string_view buf, size_t pos) {
    switch(buf[pos]) {
        case escape_marker: {
            pos++;
            if(pos >= buf.size()) {
                throw std::runtime_error("Unfinished antislash escape sequence");
            }
            switch(buf[pos]) {
                case '\\': return std::tuple<std::string_view, size_t>("\\", pos+1);
                case 'n':  return std::tuple<std::string_view, size_t>("\n", pos+1);
                case 't':  return std::tuple<std::string_view, size_t>("\t", pos+1);
                case 'r':  return std::tuple<std::string_view, size_t>("\r", pos+1);
            }
            throw std::runtime_error("Invalid antislash escape sequence");
        }
        case variable_begin: {
            pos++;
...
format_pattern::format_pattern(std::string&& pattern): _buffer(std::make_shared<std::string>(std::forward<std::string>(pattern))) {
    ...
    while(cursor < buf.size()) {
        // TODO what about non-print chars ?
        ...
        cursor = parse_variable(buf, cursor + 1);
